...
class BenchFlowFlowsTestCase(unittest.TestCase):
    ...
    def create_order(self, extra_files=None):
        payload = {
            "customer_name": "Klaus Winter",
            "customer_phone": "+49 171 5550000",
            "customer_email": "klaus@example.de",
            "category": "amp",
            "manufacturer": "Mesa",
            "model": "Boogie",
    ...
    def test_privacy_page_renders(self):
        response = self.get("/datenschutz")
        body = response.get_data(as_text=True)

        self.assertEqual(response.status_code, 200)
        self.assertIn("Datenschutz", body)
        self.assertIn("DSGVO", body)

    def test_imprint_page_renders(self):
        response = self.get("/impressum")
        body = response.get_data(as_text=True)

        self.assertEqual(response.status_code, 200)
        self.assertIn("Impressum", body)
        self.assertIn("§ 5 TMG", body)
    ...
    def test_extended_customer_details_are_saved_and_rendered(self):
        response = self.post(
            ...
            data={
                "customer_name": "Klaus Winter",
                "company_name": "Winter Audio GmbH",
                "customer_phone": "+49 171 5550000",
                "customer_email": "klaus@example.de",
                "street": "Studioallee 12",
                "postal_code": "50667",
                "city": "Koeln",
                "preferred_contact": "email",
                "customer_notes": "Bitte zuerst per Mail kontaktieren.",
                "category": "amp",
                "manufacturer": "Mesa",
                "model": "Boogie",
                "serial_number": "MB-123",
        ...
        self.assertIn("Winter Audio GmbH", detail)
        self.assertIn("Studioallee 12", detail)
        self.assertIn("50667", detail)
        self.assertIn("Koeln", detail)
        self.assertIn("Bitte zuerst per Mail kontaktieren.", detail)
        self.assertIn("Winter Audio GmbH", print_view)
        self.assertIn("Studioallee 12", print_view)
    ...
    def test_new_order_page_renders_customer_lookup_with_existing_customers(self):
        ...
        self.assertIn("Klaus Winter | Winter Audio GmbH | +49 171 5550000", new_order_page)

    def test_new_order_page_renders_manufacturer_dropdown(self):
        new_order_page = self.get("/orders/new").get_data(as_text=True)

        self.assertIn('<select name="manufacturer"', new_order_page)
        self.assertIn(">Fender</option>", new_order_page)
        self.assertIn(">Mesa</option>", new_order_page)

    def test_demo_seed_contains_quotes_invoices_and_more_orders(self):
        with self.app_db_context():
            db = get_db()
            order_count = db.execute("SELECT COUNT(*) AS total FROM service_orders").fetchone()["total"]
            quote_count = db.execute("SELECT COUNT(*) AS total FROM service_order_quotes").fetchone()["total"]
            invoice_count = db.execute("SELECT COUNT(*) AS total FROM service_order_invoices").fetchone()["total"]
            media_count = db.execute(
                """
                SELECT COUNT(*) AS total
                FROM service_order_media
                WHERE document_type IN ('kva', 'rechnung')
                """
            ).fetchone()["total"]
            completed_demo = db.execute(
                """
                SELECT so.order_number, q.quote_number, i.invoice_number
                FROM service_orders so
                LEFT JOIN service_order_quotes q ON q.service_order_id = so.id
                LEFT JOIN service_order_invoices i ON i.service_order_id = so.id
                WHERE so.order_number = 'SO-1006'
                """
            ).fetchone()

        self.assertGreaterEqual(order_count, 36)
        self.assertGreaterEqual(quote_count, 3)
        self.assertGreaterEqual(invoice_count, 2)
        self.assertGreaterEqual(media_count, 5)
        self.assertEqual(completed_demo["quote_number"], "KVA-1006")
        self.assertEqual(completed_demo["invoice_number"], "RE-1006")
    ...
    def test_customer_detail_page_shows_customer_data_and_orders(self):
        ...
        with self.app_db_context():
            ...
            created_order_number = order_row["order_number"]
        ...
        self.assertIn("Winter Audio GmbH", body)
        self.assertIn("Studioallee 12", body)
        self.assertIn("Bitte zuerst per Mail kontaktieren.", body)
        self.assertIn(created_order_number, body)

    def test_order_detail_page_renders_manufacturer_dropdown(self):
        response = self.create_order()
        order_id = int(response.headers["Location"].rsplit("/", 1)[-1])

        detail_page = self.get(f"/orders/{order_id}").get_data(as_text=True)

        self.assertIn('<select name="manufacturer"', detail_page)
        self.assertIn('value="Mesa" selected', detail_page)
        self.assertIn(">Fender</option>", detail_page)
    ...
    def test_quote_email_preparation_redirects_to_mailto_and_logs_history(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertTrue(response.headers["Location"].startswith("mailto:klaus%40example.de"))
        self.assertIn("Ihr%20Kostenvoranschlag%20KVA-", response.headers["Location"])
        self.assertIn("Ger%C3%A4t%3A%20Mesa%20Boogie", response.headers["Location"])
        ...
        self.assertIn("KVA-Mail an klaus@example.de vorbereitet.", history["note"])
    ...
    def test_pdf_document_type_is_saved_and_rendered(self):
        response = self.post(
            ...
            data={
                ...
                "diagnostic_source": "Werkbank",
                "document_type": "kva",
                "document_context": "KVA für Endstufentausch",
                "internal_notes": "Externer KVA erwartet.",
                "order_files": [
                    (
                        BytesIO(b"%PDF-1.4\n1 0 obj\n<< /Type /Catalog >>\nendobj\ntrailer\n<< /Root 1 0 R >>\n%%EOF"),
        ...
        self.assertEqual(row["order_context"], "KVA für Endstufentausch")
        detail = self.get(f"/orders/{order_id}")
        self.assertIn("Kostenvoranschlag", detail.get_data(as_text=True))
        self.assertIn("KVA für Endstufentausch", detail.get_data(as_text=True))
    ...
    def test_existing_pdf_document_type_can_be_changed_on_detail_page(self):
        ...
        update_response = self.post(
            ...
            data={
                ...
                "document_context": "",
                "internal_notes": "Externer KVA erwartet.",
                f"document_name_{media_id}": "rechnung-externe-reparatur",
        ...
        with self.app_db_context():
            db = get_db()
            row = db.execute(
                "SELECT filename, document_type, order_context FROM service_order_media WHERE id = ?",
                (media_id,),
            ).fetchone()

        self.assertEqual(row["filename"], "rechnung-externe-reparatur.pdf")
        ...
        self.assertIn("rechnung-externe-reparatur.pdf", detail.get_data(as_text=True))
    ...
    def test_orders_list_marks_mixed_document_types(self):
        ...
        self.post(
            ...
            data={
                ...
                f"document_context_{media_id}": "KVA für Endstufentausch",
                "order_files": [
                    (
                        BytesIO(b"%PDF-1.4\n1 0 obj\n<< /Type /Catalog >>\nendobj\ntrailer\n<< /Root 1 0 R >>\n%%EOF"),
                        "rechnung.pdf",
